    ...
    public abstract class PayPalResource : PayPalSerializableObject
    {
        ...
        protected internal static T ConfigureAndExecute<T>(APIContext apiContext, HttpMethod httpMethod, string resource, string payload = "", string endpoint = "", bool setAuthorizationHeader = true)
        {
            ...
            try
            {
                ...
                if (!Uri.TryCreate(baseUri, resource, out Uri uniformResourceIdentifier))
                {
            ...
            catch (ConnectionException ex)
            {
                if (ex is HttpException)
                {
                    ...
                    {
                        if (httpEx.TryConvertTo<PaymentsException>(out PaymentsException paymentsEx))
                        {
                            throw paymentsEx;
                        }
                    }
                    ...
                    {
                        if (httpEx.TryConvertTo<IdentityException>(out IdentityException identityEx))
                        {
                            throw identityEx;
                        }
                    }
